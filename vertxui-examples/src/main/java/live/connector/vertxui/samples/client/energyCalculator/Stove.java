...
public class Stove {
	...
	public Stove(Fluent body) {
		...
		body.add(new InputNumber().att(Att.value, power + "").keyup((fluent, ___) -> {
			power = ((InputNumber) fluent).domValueDouble();
			conclusion.sync();
		}));
		...
		body.select(null, new String[] { "normal (4222 w/kg)", "4220", "1 summer (3400 w/kg)", "3400",
